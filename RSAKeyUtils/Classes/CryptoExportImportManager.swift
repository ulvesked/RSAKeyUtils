...
@available(iOS 10.0, *)
public class CryptoExportImportManager {
    
    
    public static func exportPublicKeyToPEM(_ pubKey: SecKey) -> String? {
        let pubKeyData = SecKeyCopyExternalRepresentation(pubKey, nil);
        let encodedKey = CryptoExportImportManager.exportPublicKeyToDER(pubKeyData! as NSData as Data);
        return CryptoExportImportManager.PEMKeyFromDERKey(encodedKey, prefix: kCryptoExportImportManagerPublicKeyInitialTag, suffix: kCryptoExportImportManagerPublicKeyFinalTag);
    }
    
    public static func exportPrivateKeyToPEM(_ privKey: SecKey) -> String? {
     ...
     */
    public static func exportPublicKeyToDER(_ rawPublicKeyBytes: Data) -> Data {
